...
public class FPS_InputReload: StateBehaviour
{
    [Header("按键模式")]
    ...
    private void Update()
    {
        switch (keyMode) {
            ...
            if (Input.GetKey(key) & InterruptAction & WeaponsManager.Instance.UseWeapon.total > 0 &
            ...
            break;
            case KeyMode.Down:
            if (Input.GetKeyDown(key) & InterruptAction & WeaponsManager.Instance.UseWeapon.total > 0 &
                WeaponsManager.Instance.UseWeapon.quantity != WeaponsManager.Instance.UseWeapon.cartrNumber) {
                InputControl.Instance.downFire = true;
                stateManager.OnEnterNextState(stateMachine.currState, state);
            }
