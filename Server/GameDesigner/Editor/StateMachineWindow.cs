	...
	public class StateMachineWindow : GraphEditor
	{
        ...
        public static StateMachineWindow Instance{
			get{ 
				if( BlueprintGUILayout.Instance.GraphEditor == null ){
					BlueprintGUILayout.Instance.GraphEditor = GetWindow <StateMachineWindow> ( "游戏设计师编辑器窗口" , true );
				}
		...
		private void CreateStateMachineMenu()
		{
			if( currentType == EventType.MouseDown & Event.current.button == 1 ){
				...
				menu.AddItem ( new GUIContent("创建状态机") , false , delegate {
					if( Selection.activeGameObject == null ){
						EditorUtility.DisplayDialog("提示!", "请选择物体后在点击创建状态机！", "是", "否");
		...
		{
            stateMachine.gameObject.hideFlags = BlueprintGUILayout.Instance.StateHideFlags;
            foreach (var state in stateMachine.states){
                ...
                if (state.rect.Contains(Event.current.mousePosition) & currentType == EventType.MouseDown & Event.current.button == 0) {
                    ...
                    switch (BlueprintGUILayout.Instance.selectObjModel) {
                        case SelectionObjModel.SelectionStateManager:
                        Selection.activeObject = stateMachine.stateManager;
                        break;
                        case SelectionObjModel.SelectionStateMachine:
                        Selection.activeObject = stateMachine;
                        break;
                        case SelectionObjModel.SelectionStateObject:
		...
		protected void OpenStateContextMenu( State state )
		{
			...
			if (currentType == EventType.MouseDown & currentEvent.button == 0) {
				...
				GenericMenu menu = new GenericMenu ();
				menu.AddItem (new GUIContent ("创建过渡"), false, delegate { 
					state.makeTransition = true; 
				} );
				menu.AddSeparator ("");
				menu.AddItem (new GUIContent ("默认状态"), false, delegate {
					stateMachine.defaultState = state;
				});
				menu.AddSeparator ("");
				menu.AddItem (new GUIContent ("复制状态"), false, delegate {
					stateMachine.selectState = state;
				});
				menu.AddItem (new GUIContent ("删除状态"), false, delegate { DeletedState(); } );
				menu.ShowAsContext ();
                Event.current.Use();
            }
		}
			...
			if( currentType == EventType.MouseDown ){
				if( currentEvent.button == 1 ){
					...
					menu.AddItem ( new GUIContent("创建状态") , false ,  
						delegate 
						{
							State s = State.CreateStateInstance( stateMachine , "新的状态 " + stateMachine.states.Count , mousePosition );
					...
					if(stateMachine.selectState!=null){
						menu.AddItem ( new GUIContent("粘贴选择状态") , false ,
                            delegate {
						...
						menu.AddItem (new GUIContent ("删除选择状态"), false, delegate { DeletedState (); });
					}
					menu.AddSeparator ("");
					menu.AddItem ( new GUIContent("创建并替换状态机") , false , delegate
                    {
                        if (Selection.activeGameObject == null) {
                            EditorUtility.DisplayDialog("提示!", "请选择物体后在点击创建状态机！", "是", "否");
                            return;
					...
					menu.AddItem ( new GUIContent("创建并更换状态机") , false , ()=>
                    {
                        if (Selection.activeGameObject == null) {
                            EditorUtility.DisplayDialog("提示!", "请选择物体后在点击创建状态机！", "是", "否");
                            return;
                        ...
                        if (manager == null)
                            manager = Selection.activeGameObject.AddComponent<StateManager>();
                        StateMachine machine = StateMachine.CreateStateMachineInstance("new 状态机");
                        Undo.RegisterCreatedObjectUndo(machine.gameObject, machine.name);
                        manager.stateMachine = machine;
                        machine.transform.SetParent(manager.transform);
                    });
					menu.AddItem ( new GUIContent("删除状态机") , false , ()=>
                    {
                        if (stateMachine == null)
                            return;
                        Undo.DestroyObjectImmediate(stateMachine.gameObject);
                    });
                    menu.AddItem(new GUIContent("删除状态管理器"), false, ()=>
                    {
                        if (stateMachine == null)
                            return;
