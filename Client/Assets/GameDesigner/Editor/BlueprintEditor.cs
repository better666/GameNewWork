...
namespace GameDesigner
{
	public class BlueprintEditor : GraphEditor
	{
		...
		static public BlueprintEditor Instance{
			get{
				if( BlueprintGUILayout.Instance.BlueprintEditor == null ){
					...
					switch(BlueprintGUILayout.Instance.initializeBlueprintEditor){
					case InitializeBlueprintEditorMode.FirstOpenBlueprintUpdate:
						InitSetDllFiles ();
						BlueprintGUILayout.Instance.initializeBlueprintEditor = InitializeBlueprintEditorMode.NoSetting;
						break;
					case InitializeBlueprintEditorMode.OpenBlueprintUpdate:
						InitSetDllFiles ();
						break;
					}
				}
		...
		/// <summary>
		/// 初始化加载Assembly-CSharp.csproj文件中包含的所有引用dll文件的路径，复制所有dll文件到同Assets文件夹的一个Managed文件夹下
		/// </summary>
		private static void InitSetDllFiles()
		{
			if (!System.IO.Directory.Exists(Application.dataPath.Replace("Assets", "Managed"))) {
				System.IO.Directory.CreateDirectory(Application.dataPath.Replace("Assets", "Managed"));
			}
			try{
                foreach (var file in System.IO.Directory.GetFiles(Application.dataPath.Replace("Assets", ""), "*csproj")) {
                    string[] dllPaths = System.IO.File.ReadAllLines(file);
                    foreach (var dllPath in dllPaths) {
                        if (dllPath.Contains("<HintPath>")) {
                            string path = dllPath.Replace("<HintPath>", "");
                            path = path.Replace("</HintPath>", "");
                            path = path.Replace("\\", "/");
                            path = path.Trim();
                            try {
                                System.IO.File.Copy(path, Application.dataPath.Replace("Assets", "Managed/") + ConvertUtility.StringSplitEndOne(path, '/'), true);
                            } catch { Debug.Log("获取" + path + "文件失败，关闭蓝图编辑器后在打开蓝图编辑器直到获取成功为止！"); }
                        }
                    }
                }
				For(Application.dataPath,"");
				try{
					System.IO.File.Copy(System.Reflection.Assembly.Load("mscorlib").GetType("System.String").Module.FullyQualifiedName,Application.dataPath.Replace("Assets", "Managed/mscorlib.dll"),true);
				}catch{Debug.Log("获取mscorlib.dll(C#基础核心动态链接库)文件失败，请自行查找其dll文件放入Plugins/Managed文件夹内!");}
			}catch{ Debug.Log ("Assembly-CSharp.csproj文件不存在，你新建个C#脚本进入monoDevelop代码编辑器后自动生成，回到编辑器重新打开蓝图编辑器即可解决！"); }
		}

		/// <summary>
		/// 循环搜索所有子文件夹路径的dll文件
		/// </summary>
		private static void For( string path , string contains)
		{
			foreach( var file in System.IO.Directory.GetFiles(path,"*dll") ){ //获取当前文件夹的dll文件
				if (contains == "" | ConvertUtility.StringSplitEndOne (file, '\\').Contains (contains)) {
					try{
						System.IO.File.Copy (file, Application.dataPath.Replace ("Assets", "Managed/") + ConvertUtility.StringSplitEndOne (file, '\\'), true);
					}catch {Debug.Log("获取"+path+"文件失败，关闭蓝图编辑器后在打开蓝图编辑器直到获取成功为止！");}
				}
			}
